...
public class BulletObjectWithMarker : BulletObject
{
	...
	protected bool _isWaitMarkerLife;

	protected float _markerChargeAfterSec;

	protected float _elapsedMarkerCharge;
	...
	public virtual bool Initialize(CharacterBase owner, CommonObjectStatus target, float waitCount, float delay, float bulletDuration, int traceDelay, string hitAttrLabel, MarkerData markerData, Vector3 position, Quaternion rotation, float tempoScale = 1f, bool useLegacyClearMarker = false, bool isWaitMarkerLife = false)
	{
		return default(bool);
	}
