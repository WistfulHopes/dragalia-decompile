...
public class EffectManager : FastUpdateMonoBehaviour
{
	...
	public EffectObject Play(string effectName, Vector3 pos, Quaternion rot, Vector3 scale, [Optional] GameObject go, float duration = 0f, int trigger = 0, [Optional] CommonObjectStatus owner, EffectObject.EraseType eraseType = EffectObject.EraseType.DELETE, float dissapearTime = 0f, bool isStopOnlyOnce = false)
	{
		return null;
	}

	public EffectObject PlayNotDependedMaster(string effectName, Vector3 pos, Quaternion rot, Vector3 localScale, GameObject go, float duration, EffectObject.EraseType eraseType, float dissapearTime, int trigger = 0, [Optional] CommonObjectStatus owner, bool removeFromCharacterEffectWhenStop = false, bool isStopOnlyOnce = false)
	{
		return null;
	}
