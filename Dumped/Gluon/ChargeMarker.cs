...
public class ChargeMarker : CommonObjectStatus
{
	...
	public static ChargeMarker Create(eType shape, eColorType color, Vector3 pos, float rotY, float length, float width, float chargeSec, float chargeAfterSec_ = 0f, float traceSec = 0f, TraceType trace = TraceType.None, bool sudden = false, float tempoScale = 1f, [Optional] CharacterBase owner)
	{
		return null;
	}
	...
	private void SetParam_E(eColorType color, Vector3 pos, float rotY, float length, float width, float chargeSec, float chargeAfterSec_, float traceSec, TraceType trace, bool sudden, float tempoScale = 1f)
	{
	}
