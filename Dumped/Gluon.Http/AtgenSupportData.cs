namespace Gluon.Http;

public class AtgenSupportData
{
	public ulong viewer_id;

	public string name;

	public int is_friend;

	public CharaList chara_data;

	public DragonList dragon_data;

	public WeaponList weapon_data;

	public AmuletList amulet_data;

	public AmuletList amulet_2_data;

	public GameWeaponSkin weapon_skin_data;

	public GameWeaponBody weapon_body_data;

	public GameAbilityCrest[] crest_slot_type_1_crest_list;

	public GameAbilityCrest[] crest_slot_type_2_crest_list;

	public GameAbilityCrest[] crest_slot_type_3_crest_list;

	public TalismanList talisman_data;

	public WeaponPassiveAbilityList[] game_weapon_passive_ability_list;
}
